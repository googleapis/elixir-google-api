...
defmodule GoogleApi.Recommender.V1beta1.Api.Organizations do
  ...
  *   `{:error, info}` on failure
  """
  @spec recommender_organizations_locations_list(
          Tesla.Env.client(),
          String.t(),
          keyword(),
          keyword()
        ) ::
          {:ok, GoogleApi.Recommender.V1beta1.Model.GoogleCloudLocationListLocationsResponse.t()}
          | {:ok, Tesla.Env.t()}
          | {:ok, list()}
          | {:error, any()}
  def recommender_organizations_locations_list(
        connection,
        name,
        optional_params \\ [],
        opts \\ []
    ...
    |> Response.decode(
      opts ++
        [struct: %GoogleApi.Recommender.V1beta1.Model.GoogleCloudLocationListLocationsResponse{}]
    )
  end
  ...
  *   `{:error, info}` on failure
  ...
  def recommender_organizations_locations_recommenders_recommendations_mark_dismissed(
    ...
    request =
      Request.new()
      |> Request.method(:post)
      |> Request.url("/v1beta1/{+name}:markDismissed", %{
        "name" => URI.encode(name, &URI.char_unreserved?/1)
      })
      |> Request.add_optional_params(optional_params_config, optional_params)
      |> Request.library_version(@library_version)
