...
defmodule GoogleApi.Drive.V3.Api.Permissions do
  ...
  *   `optional_params` (*type:* `keyword()`) - Optional parameters
      ...
      *   `:enforceSingleParent` (*type:* `boolean()`) - Set to true to opt in to API behavior that aims for all items to have exactly one parent. This parameter will only take effect if the item is not in a shared drive. See moveToNewOwnersRoot for details.
