...
defmodule GoogleApi.Script.V1.Model.Operation do
  ...
  - done (boolean()): This field indicates whether the script execution has completed. A completed execution has a populated `response` field containing the ExecutionResponse from function that was executed. Defaults to `nil`.
  - error (GoogleApi.Script.V1.Model.Status.t): If a `run` call succeeds but the script function (or Apps Script itself) throws an exception, this field contains a Status object. The `Status` object's `details` field contains an array with a single ExecutionError object that provides information about the nature of the error. Defaults to `nil`.
  - response (map()): If the script function returns successfully, this field contains an ExecutionResponse object with the function's return value. Defaults to `nil`.
  """
