...
defmodule GoogleApi.Blogger.V3.Api.Blogs do
  ...
  *   `{:error, info}` on failure
  ...
  def blogger_blogs_get(connection, blog_id, optional_params \\ [], opts \\ []) do
    ...
    request =
      Request.new()
      |> Request.method(:get)
      |> Request.url("/v3/blogs/{blogId}", %{
        "blogId" => URI.encode(blog_id, &(URI.char_unreserved?(&1) || &1 == ?/))
      })
  ...
  *   `optional_params` (*type:* `keyword()`) - Optional parameters
      ...
      *   `:uploadType` (*type:* `String.t`) - Legacy upload protocol for media (e.g. "media", "multipart").
      *   `:upload_protocol` (*type:* `String.t`) - Upload protocol for media (e.g. "raw", "multipart").
      *   `:fetchUserInfo` (*type:* `boolean()`) - 
      *   `:role` (*type:* `list(String.t)`) - 
      *   `:status` (*type:* `list(String.t)`) - Default value of status is LIVE.
      *   `:view` (*type:* `String.t`) - 
  *   `opts` (*type:* `keyword()`) - Call options
  ...
  *   `{:error, info}` on failure
  ...
  def blogger_blogs_list_by_user(connection, user_id, optional_params \\ [], opts \\ []) do
    ...
    request =
      Request.new()
      |> Request.method(:get)
      |> Request.url("/v3/users/{userId}/blogs", %{
