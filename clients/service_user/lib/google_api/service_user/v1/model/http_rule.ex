...
defmodule GoogleApi.ServiceUser.V1.Model.HttpRule do
  ...
  - additionalBindings (List[HttpRule]): Additional HTTP bindings for the selector. Nested bindings must not contain an &#x60;additional_bindings&#x60; field themselves (that is, the nesting may only be one level deep). Defaults to: `null`.
  - authorizations (List[AuthorizationRule]): Specifies the permission(s) required for an API element for the overall API request to succeed. It is typically used to mark request message fields that contain the name of the resource and indicates the permissions that will be checked on that resource. Defaults to: `null`.
  - body (String): The name of the request field whose value is mapped to the HTTP body, or &#x60;*&#x60; for mapping all fields not captured by the path pattern to the HTTP body. NOTE: the referred field must not be a repeated field and must be present at the top-level of request message type. Defaults to: `null`.
  ...
  - delete (String): Used for deleting a resource. Defaults to: `null`.
  - get (String): Used for listing and getting information about resources. Defaults to: `null`.
  ...
  - patch (String): Used for updating a resource. Defaults to: `null`.
  - post (String): Used for creating a resource. Defaults to: `null`.
  - put (String): Used for updating a resource. Defaults to: `null`.
  - responseBody (String): The name of the response field whose value is mapped to the HTTP body of response. Other response fields are ignored. This field is optional. When not set, the response message will be used as HTTP body of response. NOTE: the referred field must be not a repeated field and must be present at the top-level of response message type. Defaults to: `null`.
  - restCollection (String): DO NOT USE. This is an experimental field.  Optional. The REST collection name is by default derived from the URL pattern. If specified, this field overrides the default collection name. Example:      rpc AddressesAggregatedList(AddressesAggregatedListRequest)         returns (AddressesAggregatedListResponse) {       option (google.api.http) &#x3D; {         get: \&quot;/v1/projects/{project_id}/aggregated/addresses\&quot;         rest_collection: \&quot;projects.addresses\&quot;       };     }  This method has the automatically derived collection name \&quot;projects.aggregated\&quot;. Because, semantically, this rpc is actually an operation on the \&quot;projects.addresses\&quot; collection, the &#x60;rest_collection&#x60; field is configured to override the derived collection name. Defaults to: `null`.
  - restMethodName (String): DO NOT USE. This is an experimental field.  Optional. The rest method name is by default derived from the URL pattern. If specified, this field overrides the default method name. Example:      rpc CreateResource(CreateResourceRequest)         returns (CreateResourceResponse) {       option (google.api.http) &#x3D; {         post: \&quot;/v1/resources\&quot;,         body: \&quot;resource\&quot;,         rest_method_name: \&quot;insert\&quot;       };     }  This method has the automatically derived rest method name \&quot;create\&quot;, but for backwards compatibility with apiary, it is specified as insert. Defaults to: `null`.
  - selector (String): Selects methods to which this rule applies.  Refer to selector for syntax details. Defaults to: `null`.
  """
