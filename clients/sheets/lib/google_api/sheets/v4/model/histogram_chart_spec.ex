...
defmodule GoogleApi.Sheets.V4.Model.HistogramChartSpec do
  ...
  - bucketSize (float()): By default the bucket size (the range of values stacked in a single
  ...
  - outlierPercentile (float()): The outlier percentile is used to ensure that outliers do not adversely
  affect the calculation of bucket sizes.  For example, setting an outlier
  percentile of 0.05 indicates that the top and bottom 5% of values when
  calculating buckets.  The values are still included in the chart, they will
  be added to the first or last buckets instead of their own buckets.
  Must be between 0.0 and 0.5. Defaults to `nil`.
  - series (list(GoogleApi.Sheets.V4.Model.HistogramSeries.t)): The series for a histogram may be either a single series of values to be
  bucketed or multiple series, each of the same length, containing the name
  of the series followed by the values to be bucketed for that series. Defaults to `nil`.
