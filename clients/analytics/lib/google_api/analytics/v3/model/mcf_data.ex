...
defmodule GoogleApi.Analytics.V3.Model.McfData do
  ...
  - columnHeaders ([McfDataColumnHeaders]): Column headers that list dimension names followed by the metric names. The order of dimensions and metrics is same as specified in the request. Defaults to: `null`.
  - containsSampledData (boolean()): Determines if the Analytics data contains sampled data. Defaults to: `null`.
  ...
  - rows ([[Object]]): Analytics data rows, where each row contains a list of dimension values followed by the metric values. The order of dimensions and metrics is same as specified in the request. Defaults to: `null`.
