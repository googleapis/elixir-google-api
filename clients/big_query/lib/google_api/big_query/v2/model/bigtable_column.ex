...
defmodule GoogleApi.BigQuery.V2.Model.BigtableColumn do
  ...
  - fieldName (String.t): [Optional] If the qualifier is not a valid BigQuery field identifier i.e. does not match [a-zA-Z][a-zA-Z0-9_]*, a valid identifier must be provided as the column field name and is used as field name in queries. Defaults to `nil`.
  - onlyReadLatest (boolean()): [Optional] If this is set, only the latest version of value in this column are exposed. 'onlyReadLatest' can also be set at the column family level. However, the setting at this level takes precedence if 'onlyReadLatest' is set at both levels. Defaults to `nil`.
  ...
  - qualifierString (String.t):  Defaults to `nil`.
