...
defmodule GoogleApi.BigQuery.V2.Model.QueryResponse do
  ...
  - rows ([TableRow]): An object with as many results as can be contained within the maximum permitted reply size. To get any additional rows, you can call GetQueryResults and specify the jobReference returned above. Defaults to: `null`.
  - schema (TableSchema): The schema of the results. Present only when the query completes successfully. Defaults to: `null`.
  - totalBytesProcessed (String.t): The total number of bytes processed for this query. If this query was a dry run, this is the number of bytes that would be processed if the query were run. Defaults to: `null`.
  - totalRows (String.t): The total number of rows in the complete query result set, which can be more than the number of rows in this single page of results. Defaults to: `null`.
  """
