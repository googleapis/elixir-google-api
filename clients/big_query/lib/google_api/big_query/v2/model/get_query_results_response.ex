...
defmodule GoogleApi.BigQuery.V2.Model.GetQueryResultsResponse do
  ...
  - etag (String.t): A hash of this response. Defaults to `nil`.
  ...
  - jobReference (GoogleApi.BigQuery.V2.Model.JobReference.t): Reference to the BigQuery Job that was created to run the query. This field will be present even if the original request timed out, in which case GetQueryResults can be used to read the results once the query has completed. Since this API only returns the first page of results, subsequent pages can be fetched via the same mechanism (GetQueryResults). Defaults to `nil`.
  - kind (String.t): The resource type of the response. Defaults to `bigquery#getQueryResultsResponse`.
  ...
  - totalBytesProcessed (String.t): The total number of bytes processed for this query. Defaults to `nil`.
  - totalRows (String.t): The total number of rows in the complete query result set, which can be more than the number of rows in this single page of results. Present only when the query completes successfully. Defaults to `nil`.
  """
