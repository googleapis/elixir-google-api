...
defmodule GoogleApi.Admin.Directory_v1.Model.Group do
  ...
  *   `aliases` (*type:* `list(String.t)`, *default:* `nil`) - Read-only. A list of a group's alias email addresses. To add, update, or remove a group's aliases, use the `groups.aliases` methods. If edited in a group's POST or PUT request, the edit is ignored.
