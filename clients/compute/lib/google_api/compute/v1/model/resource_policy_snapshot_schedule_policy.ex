...
defmodule GoogleApi.Compute.V1.Model.ResourcePolicySnapshotSchedulePolicy do
  ...
  - retentionPolicy (ResourcePolicySnapshotSchedulePolicyRetentionPolicy): Retention policy applied to snapshots created by this resource policy. Defaults to: `null`.
  - schedule (ResourcePolicySnapshotSchedulePolicySchedule): A Vm Maintenance Policy specifies what kind of infrastructure maintenance we are allowed to perform on this VM and when. Schedule that is applied to disks covered by this policy. Defaults to: `null`.
  - snapshotProperties (ResourcePolicySnapshotSchedulePolicySnapshotProperties): Properties with which snapshots are created such as labels, encryption keys. Defaults to: `null`.
  """
