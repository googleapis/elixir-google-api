...
defmodule GoogleApi.Compute.V1.Model.Router do
  ...
  - bgp (GoogleApi.Compute.V1.Model.RouterBgp.t): BGP information specific to this router. Defaults to `nil`.
  - bgpPeers (list(GoogleApi.Compute.V1.Model.RouterBgpPeer.t)): BGP information that must be configured into the routing stack to establish BGP peering. This information must specify the peer ASN and either the interface name, IP address, or peer IP address. Please refer to RFC4273. Defaults to `nil`.
  ...
  - interfaces (list(GoogleApi.Compute.V1.Model.RouterInterface.t)): Router interfaces. Each interface requires either one linked resource, (for example, linkedVpnTunnel), or IP address and IP address range (for example, ipRange), or both. Defaults to `nil`.
  - kind (String.t): [Output Only] Type of resource. Always compute#router for routers. Defaults to `compute#router`.
  ...
  - nats (list(GoogleApi.Compute.V1.Model.RouterNat.t)): A list of NAT services created in this router. Defaults to `nil`.
  - network (String.t): URI of the network to which this router belongs. Defaults to `nil`.
  - region (String.t): [Output Only] URI of the region where the router resides. You must specify this field as part of the HTTP request URL. It is not settable as a field in the request body. Defaults to `nil`.
