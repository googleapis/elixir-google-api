...
defmodule GoogleApi.Compute.V1.Model.PacketMirroring do
  ...
  *   `enable` (*type:* `String.t`, *default:* `nil`) - Indicates whether or not this packet mirroring takes effect. If set to FALSE, this packet mirroring policy will not be enforced on the network.
  ...
  *   `priority` (*type:* `integer()`, *default:* `nil`) - The priority of applying this configuration. Priority is used to break ties in cases where there is more than one matching rule. In the case of two rules that apply for a given Instance, the one with the lowest-numbered priority value wins.
