...
defmodule GoogleApi.Compute.V1.Model.BackendService do
  ...
  When the load balancing scheme is INTERNAL, this field is not used. Defaults to `nil`.
  - backends (list(GoogleApi.Compute.V1.Model.Backend.t)): The list of backends that serve this BackendService. Defaults to `nil`.
  - cdnPolicy (GoogleApi.Compute.V1.Model.BackendServiceCdnPolicy.t): Cloud CDN configuration for this BackendService. Defaults to `nil`.
  - connectionDraining (GoogleApi.Compute.V1.Model.ConnectionDraining.t):  Defaults to `nil`.
  - creationTimestamp (String.t): [Output Only] Creation timestamp in RFC3339 text format. Defaults to `nil`.
  - customRequestHeaders (list(String.t)): Headers that the HTTP/S load balancer should add to proxied requests. Defaults to `nil`.
  ...
  To see the latest fingerprint, make a get() request to retrieve a BackendService. Defaults to `nil`.
  - healthChecks (list(String.t)): The list of URLs to the HttpHealthCheck or HttpsHealthCheck resource for health checking this BackendService. Currently at most one health check can be specified, and a health check is required for Compute Engine backend services. A health check must not be specified for App Engine backend and Cloud Function backend.

  For internal load balancing, a URL to a HealthCheck resource must be specified instead. Defaults to `nil`.
  - iap (GoogleApi.Compute.V1.Model.BackendServiceIAP.t):  Defaults to `nil`.
  - id (String.t): [Output Only] The unique identifier for the resource. This identifier is defined by the server. Defaults to `nil`.
  - kind (String.t): [Output Only] Type of resource. Always compute#backendService for backend services. Defaults to `compute#backendService`.
  - loadBalancingScheme (String.t): Indicates whether the backend service will be used with internal or external load balancing. A backend service created for one type of load balancing cannot be used with the other. Possible values are INTERNAL and EXTERNAL. Defaults to `nil`.
  ...
  This cannot be used for internal load balancing. Defaults to `nil`.
  - portName (String.t): Name of backend port. The same name should appear in the instance groups referenced by this service. Required when the load balancing scheme is EXTERNAL.

  When the load balancing scheme is INTERNAL, this field is not used. Defaults to `nil`.
  - protocol (String.t): The protocol this BackendService uses to communicate with backends.

  Possible values are HTTP, HTTPS, TCP, and SSL. The default is HTTP.

  For internal load balancing, the possible values are TCP and UDP, and the default is TCP. Defaults to `nil`.
  - region (String.t): [Output Only] URL of the region where the regional backend service resides. This field is not applicable to global backend services. You must specify this field as part of the HTTP request URL. It is not settable as a field in the request body. Defaults to `nil`.
  - securityPolicy (String.t): [Output Only] The resource URL for the security policy associated with this backend service. Defaults to `nil`.
  - selfLink (String.t): [Output Only] Server-defined URL for the resource. Defaults to `nil`.
  - sessionAffinity (String.t): Type of session affinity to use. The default is NONE.

  When the load balancing scheme is EXTERNAL, can be NONE, CLIENT_IP, or GENERATED_COOKIE.

  When the load balancing scheme is INTERNAL, can be NONE, CLIENT_IP, CLIENT_IP_PROTO, or CLIENT_IP_PORT_PROTO.

  When the protocol is UDP, this field is not used. Defaults to `nil`.
  - timeoutSec (integer()): How many seconds to wait for the backend before considering it a failed request. Default is 30 seconds. Defaults to `nil`.
  """
