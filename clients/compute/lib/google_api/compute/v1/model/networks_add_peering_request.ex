...
defmodule GoogleApi.Compute.V1.Model.NetworksAddPeeringRequest do
  ...
  - autoCreateRoutes (boolean()): This field will be deprecated soon. Use exchange_subnet_routes in network_peering instead. Whether Google Compute Engine manages the routes automatically. Defaults to: `null`.
  - name (String.t): Name of the peering, which should conform to RFC1035. Defaults to: `null`.
  - networkPeering (NetworkPeering): Network peering parameters. In order to specify route policies for peering using import and export custom routes, you must specify all peering related parameters (name, peer network, exchange_subnet_routes) in the network_peering field. The corresponding fields in NetworksAddPeeringRequest will be deprecated soon. Defaults to: `null`.
