...
defmodule GoogleApi.Compute.V1.Model.DiskInstantiationConfig do
  ...
  - customImage (String.t): The custom source image to be used to restore this disk when instantiating this instance template. Defaults to `nil`.
  - deviceName (String.t): Specifies the device name of the disk to which the configurations apply to. Defaults to `nil`.
  - instantiateFrom (String.t): Specifies whether to include the disk and what image to use. Possible values are:  
  ...
  - do-not-include: to exclude a disk from the template. Applicable to additional read-write disks, local SSDs, and read-only disks. Defaults to `nil`.
  """
