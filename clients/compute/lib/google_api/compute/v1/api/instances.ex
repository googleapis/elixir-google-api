...
defmodule GoogleApi.Compute.V1.Api.Instances do
  ...
  *   `optional_params` (*type:* `keyword()`) - Optional parameters
      ...
      *   `:quotaUser` (*type:* `String.t`) - An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
      *   `:userIp` (*type:* `String.t`) - Deprecated. Please use quotaUser instead.
      *   `:minimalAction` (*type:* `String.t`) - If specified, this action or higher level action is performed on the instance irrespective of what action is required for the update to take effect. If not specified, then Compute Engine acts based on the minimum action required.
      *   `:mostDisruptiveAllowedAction` (*type:* `String.t`) - If specified, Compute Engine returns an error if the update requires a higher action to be applied to the instance. If not specified, the default will be REFRESH.
      *   `:requestId` (*type:* `String.t`) - An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed.
  ...
  *   `{:error, info}` on failure
  ...
  def compute_instances_update_display_device(
    ...
    request =
      Request.new()
      |> Request.method(:patch)
      |> Request.url("/{project}/zones/{zone}/instances/{instance}/updateDisplayDevice", %{
        "project" => URI.encode(project, &URI.char_unreserved?/1),
        "zone" => URI.encode(zone, &URI.char_unreserved?/1),
        "instance" => URI.encode(instance, &URI.char_unreserved?/1)
      })
