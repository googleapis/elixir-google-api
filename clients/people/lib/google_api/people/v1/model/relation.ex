...
defmodule GoogleApi.People.V1.Model.Relation do
  ...
  *   `formattedType` (*type:* `String.t`, *default:* `nil`) - Output only. The type of the relation translated and formatted in the viewer's account locale or the locale specified in the Accept-Language HTTP header.
  ...
  *   `type` (*type:* `String.t`, *default:* `nil`) - The person's relation to the other person. The type can be custom or one of these predefined values: * `spouse` * `child` * `mother` * `father` * `parent` * `brother` * `sister` * `friend` * `relative` * `domesticPartner` * `manager` * `assistant` * `referredBy` * `partner`
  """
