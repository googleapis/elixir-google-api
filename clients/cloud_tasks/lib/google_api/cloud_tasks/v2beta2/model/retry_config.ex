...
defmodule GoogleApi.CloudTasks.V2beta2.Model.RetryConfig do
  ...
  `max_attempts - 1` retries).  Must be > 0. Defaults to `nil`.
  - maxBackoff (String.t): A task will be scheduled for retry between
  min_backoff and
  ...
  queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters). Defaults to `nil`.
  ...
  [max_doublings in
  queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters). Defaults to `nil`.
  - maxRetryDuration (String.t): If positive, `max_retry_duration` specifies the time limit for
  ...
  [task_age_limit in
  queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters). Defaults to `nil`.
  - minBackoff (String.t): A task will be scheduled for retry between
  min_backoff and
  max_backoff duration after it fails,
  if the queue's RetryConfig specifies that the task should be
  retried.
  ...
  queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters). Defaults to `nil`.
  - unlimitedAttempts (boolean()): If true, then the number of attempts is unlimited. Defaults to `nil`.
  """
