...
defmodule GoogleApi.CloudTasks.V2beta2.Model.RetryConfig do
  ...
  *   `maxAttempts` (*type:* `integer()`, *default:* `nil`) - The maximum number of attempts for a task.

      Cloud Tasks will attempt the task `max_attempts` times (that
      is, if the first attempt fails, then there will be
      `max_attempts - 1` retries).  Must be > 0.
  *   `maxBackoff` (*type:* `String.t`, *default:* `nil`) - A task will be scheduled for retry between
      min_backoff and
  ...
  *   `maxDoublings` (*type:* `integer()`, *default:* `nil`) - The time between retries will double `max_doublings` times.
      ...
      [max_doublings in
      queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters).
  *   `maxRetryDuration` (*type:* `String.t`, *default:* `nil`) - If positive, `max_retry_duration` specifies the time limit for
      ...
      [task_age_limit in
      queue.yaml/xml](https://cloud.google.com/appengine/docs/standard/python/config/queueref#retry_parameters).
  *   `minBackoff` (*type:* `String.t`, *default:* `nil`) - A task will be scheduled for retry between
      min_backoff and
      max_backoff duration after it fails,
      if the queue's RetryConfig specifies that the task should be
      retried.
