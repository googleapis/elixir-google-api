...
defmodule GoogleApi.CloudDebugger.V2.Model.ListActiveBreakpointsResponse do
  ...
  *   `breakpoints` (*type:* `list(GoogleApi.CloudDebugger.V2.Model.Breakpoint.t)`, *default:* `nil`) - List of all active breakpoints. The fields `id` and `location` are guaranteed to be set on each breakpoint.
  ...
  *   `waitExpired` (*type:* `boolean()`, *default:* `nil`) - If set to `true`, indicates that there is no change to the list of active breakpoints and the server-selected timeout has expired. The `breakpoints` field would be empty and should be ignored.
  """
