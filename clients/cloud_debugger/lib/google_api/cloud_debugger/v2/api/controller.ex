...
defmodule GoogleApi.CloudDebugger.V2.Api.Controller do
  ...
  *   `optional_params` (*type:* `keyword()`) - Optional parameters
      ...
      *   `:uploadType` (*type:* `String.t`) - Legacy upload protocol for media (e.g. "media", "multipart").
      *   `:upload_protocol` (*type:* `String.t`) - Upload protocol for media (e.g. "raw", "multipart").
      *   `:agentId` (*type:* `String.t`) - Identifies the agent. This is the ID returned in the RegisterDebuggee response.
      *   `:successOnTimeout` (*type:* `boolean()`) - If set to `true` (recommended), returns `google.rpc.Code.OK` status and sets the `wait_expired` response field to `true` when the server-selected timeout has expired. If set to `false` (deprecated), returns `google.rpc.Code.ABORTED` status when the server-selected timeout has expired.
      *   `:waitToken` (*type:* `String.t`) - A token that, if specified, blocks the method call until the list of active breakpoints has changed, or a server-selected timeout has expired. The value should be set from the `next_wait_token` field in the last response. The initial value should be set to `"init"`.
  *   `opts` (*type:* `keyword()`) - Call options
