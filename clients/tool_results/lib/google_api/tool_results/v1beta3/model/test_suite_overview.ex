...
defmodule GoogleApi.ToolResults.V1beta3.Model.TestSuiteOverview do
  ...
  - In create/response: always set - In update request: never Defaults to `nil`.
  ...
  - In create/response: always set - In update request: never Defaults to `nil`.
  - name (String.t): The name of the test suite.
  ...
  - In create/response: always set - In update request: never Defaults to `nil`.
  - xmlSource (GoogleApi.ToolResults.V1beta3.Model.FileReference.t): If this test suite was parsed from XML, this is the URI where the original XML file is stored.

  Note: Multiple test suites can share the same xml_source

  Returns INVALID_ARGUMENT if the uri format is not supported.

  - In create/response: optional - In update request: never Defaults to `nil`.
  """
