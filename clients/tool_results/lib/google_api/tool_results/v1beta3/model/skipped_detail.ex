...
defmodule GoogleApi.ToolResults.V1beta3.Model.SkippedDetail do
  ...
  - incompatibleAppVersion (boolean()): If the App doesn't support the specific API level. Defaults to `nil`.
  - incompatibleArchitecture (boolean()): If the App doesn't run on the specific architecture, for example, x86. Defaults to `nil`.
  - incompatibleDevice (boolean()): If the requested OS version doesn't run on the specific device model. Defaults to `nil`.
  """
