...
defmodule GoogleApi.ToolResults.V1beta3.Model.TestExecutionStep do
  ...
  - testIssues (list(GoogleApi.ToolResults.V1beta3.Model.TestIssue.t)): Issues observed during the test execution.
  ...
  - In response: present if set by create or update - In create/update request: optional Defaults to `nil`.
  - testSuiteOverviews (list(GoogleApi.ToolResults.V1beta3.Model.TestSuiteOverview.t)): List of test suite overview contents. This could be parsed from xUnit XML log by server, or uploaded directly by user. This references should only be called when test suites are fully parsed or uploaded.

  The maximum allowed number of test suite overviews per step is 1000.

  - In response: always set - In create request: optional - In update request: never (use publishXunitXmlFiles custom method instead) Defaults to `nil`.
  - testTiming (GoogleApi.ToolResults.V1beta3.Model.TestTiming.t): The timing break down of the test execution.

  - In response: present if set by create or update - In create/update request: optional Defaults to `nil`.
  - toolExecution (GoogleApi.ToolResults.V1beta3.Model.ToolExecution.t): Represents the execution of the test runner.
