...
defmodule GoogleApi.ToolResults.V1beta3.Model.Step do
  ...
  - completionTime (GoogleApi.ToolResults.V1beta3.Model.Timestamp.t): The time when the step status was set to complete.

  This value will be set automatically when state transitions to COMPLETE.

  - In response: set if the execution state is COMPLETE. - In create/update request: never set Defaults to `nil`.
  - creationTime (GoogleApi.ToolResults.V1beta3.Model.Timestamp.t): The time when the step was created.

  - In response: always set - In create/update request: never set Defaults to `nil`.
  - description (String.t): A description of this tool For example: mvn clean package -D skipTests=true

  - In response: present if set by create/update request - In create/update request: optional Defaults to `nil`.
  - deviceUsageDuration (GoogleApi.ToolResults.V1beta3.Model.Duration.t): How much the device resource is used to perform the test.
  ...
  - In response: present if previously set. - In create request: optional - In update request: optional Defaults to `nil`.
  - dimensionValue (list(GoogleApi.ToolResults.V1beta3.Model.StepDimensionValueEntry.t)): If the execution containing this step has any dimension_definition set, then this field allows the child to specify the values of the dimensions.
  ...
  - In response: present if set by create - In create request: optional - In update request: never set Defaults to `nil`.
  - hasImages (boolean()): Whether any of the outputs of this step are images whose thumbnails can be fetched with ListThumbnails.

  - In response: always set - In create/update request: never set Defaults to `nil`.
  - labels (list(GoogleApi.ToolResults.V1beta3.Model.StepLabelsEntry.t)): Arbitrary user-supplied key/value pairs that are associated with the step.
  ...
  - In response: always set - In create request: optional - In update request: optional; any new key/value pair will be added to the map, and any new value for an existing key will update that key's value Defaults to `nil`.
  - multiStep (GoogleApi.ToolResults.V1beta3.Model.MultiStep.t): Details when multiple steps are run with the same configuration as a group. These details can be used identify which group this step is part of. It also identifies the groups 'primary step' which indexes all the group members.

  - In response: present if previously set. - In create request: optional, set iff this step was performed more than once. - In update request: optional Defaults to `nil`.
  - name (String.t): A short human-readable name to display in the UI. Maximum of 100 characters. For example: Clean build
  ...
  - In response: always set - In create request: always set - In update request: never set Defaults to `nil`.
  - outcome (GoogleApi.ToolResults.V1beta3.Model.Outcome.t): Classification of the result, for example into SUCCESS or FAILURE

  - In response: present if set by create/update request - In create/update request: optional Defaults to `nil`.
  - runDuration (GoogleApi.ToolResults.V1beta3.Model.Duration.t): How long it took for this step to run.
  ...
  - In response: present if previously set; always present on COMPLETE step - In create request: optional - In update request: optional Defaults to `nil`.
  - state (String.t): The initial state is IN_PROGRESS. The only legal state transitions are * IN_PROGRESS -> COMPLETE
  ...
  - In response: always set - In create/update request: optional Defaults to `nil`.
  - stepId (String.t): A unique identifier within a Execution for this Step.

  Returns INVALID_ARGUMENT if this field is set or overwritten by the caller.

  - In response: always set - In create/update request: never set Defaults to `nil`.
  - testExecutionStep (GoogleApi.ToolResults.V1beta3.Model.TestExecutionStep.t): An execution of a test runner. Defaults to `nil`.
  - toolExecutionStep (GoogleApi.ToolResults.V1beta3.Model.ToolExecutionStep.t): An execution of a tool (used for steps we don't explicitly support). Defaults to `nil`.
  """
