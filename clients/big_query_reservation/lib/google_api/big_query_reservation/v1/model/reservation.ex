...
defmodule GoogleApi.BigQueryReservation.V1.Model.Reservation do
  ...
  *   `primaryLocation` (*type:* `String.t`, *default:* `nil`) - Optional. The primary location of the reservation. The field is only meaningful for a failover reservation that is used for managed disaster recovery. The field is output only for customers and should not be specified. However, the `google.api.field_behavior` is not set to `OUTPUT_ONLY` since these fields are set in rerouted requests sent across regions.
  *   `secondaryLocation` (*type:* `String.t`, *default:* `nil`) - Optional. The secondary location of the reservation that is used for managed disaster recovery. Customers can set this in create/update reservation calls to create a failover reservation or convert a non-failover reservation to a failover reservation.
