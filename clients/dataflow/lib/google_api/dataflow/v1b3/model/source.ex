...
defmodule GoogleApi.Dataflow.V1b3.Model.Source do
  ...
  - baseSpecs (list(map())): While splitting, sources may specify the produced bundles
  as differences against another source, in order to save backend-side
  memory and allow bigger jobs. For details, see SourceSplitRequest.
  To support this use case, the full set of parameters of the source
  is logically obtained by taking the latest explicitly specified value
  of each parameter in the order:
  base_specs (later items win), spec (overrides anything in base_specs). Defaults to `nil`.
  - codec (map()): The codec to use to decode data read from the source. Defaults to `nil`.
  ...
  this field populated. Defaults to `nil`.
  - metadata (GoogleApi.Dataflow.V1b3.Model.SourceMetadata.t): Optionally, metadata for this source can be supplied right away,
  ...
  Source objects supplied by the framework to the user don't have
  this field populated. Defaults to `nil`.
  - spec (map()): The source to read from, plus its parameters. Defaults to `nil`.
  """
