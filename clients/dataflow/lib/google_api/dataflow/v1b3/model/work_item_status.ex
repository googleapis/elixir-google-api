...
defmodule GoogleApi.Dataflow.V1b3.Model.WorkItemStatus do
  ...
  - counterUpdates ([CounterUpdate]): Worker output counters for this WorkItem. Defaults to: `null`.
  - dynamicSourceSplit (DynamicSourceSplit): See documentation of stop_position. Defaults to: `null`.
  - errors ([Status]): Specifies errors which occurred during processing.  If errors are provided, and completed &#x3D; true, then the WorkItem is considered to have failed. Defaults to: `null`.
  - metricUpdates ([MetricUpdate]): DEPRECATED in favor of counter_updates. Defaults to: `null`.
  - progress (ApproximateProgress): DEPRECATED in favor of reported_progress. Defaults to: `null`.
  - reportIndex (String.t): The report index.  When a WorkItem is leased, the lease will contain an initial report index.  When a WorkItem&#39;s status is reported to the system, the report should be sent with that report index, and the response will contain the index the worker should use for the next report.  Reports received with unexpected index values will be rejected by the service.  In order to preserve idempotency, the worker should not alter the contents of a report, even if the worker must submit the same report multiple times before getting back a response.  The worker should not submit a subsequent report until the response for the previous report had been received from the service. Defaults to: `null`.
  - reportedProgress (ApproximateReportedProgress): The worker&#39;s progress through this WorkItem. Defaults to: `null`.
  - requestedLeaseDuration (String.t): Amount of time the worker requests for its lease. Defaults to: `null`.
  ...
  - totalThrottlerWaitTimeSeconds (float()): Total time the worker spent being throttled by external systems. Defaults to: `null`.
  - workItemId (String.t): Identifies the WorkItem. Defaults to: `null`.
  """
  ...
  @type t :: %__MODULE__{
          ...
          :totalThrottlerWaitTimeSeconds => any(),
          :workItemId => any()
        }
