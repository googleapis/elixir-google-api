...
defmodule GoogleApi.Dataflow.V1b3.Model.StreamingConfigTask do
  ...
  - maxWorkItemCommitBytes (String.t): Maximum size for work item commit supported windmill storage layer. Defaults to `nil`.
  - streamingComputationConfigs (list(GoogleApi.Dataflow.V1b3.Model.StreamingComputationConfig.t)): Set of computation configuration information. Defaults to `nil`.
  - userStepToStateFamilyNameMap (map()): Map from user step names to state families. Defaults to `nil`.
  - windmillServiceEndpoint (String.t): If present, the worker must use this endpoint to communicate with Windmill
  Service dispatchers, otherwise the worker must continue to use whatever
  endpoint it had been using. Defaults to `nil`.
  - windmillServicePort (String.t): If present, the worker must use this port to communicate with Windmill
