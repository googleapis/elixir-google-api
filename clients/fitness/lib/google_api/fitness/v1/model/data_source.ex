...
defmodule GoogleApi.Fitness.V1.Model.DataSource do
  ...
  - application (GoogleApi.Fitness.V1.Model.Application.t): Information about an application which feeds sensor data into the platform. Defaults to `nil`.
  - dataQualityStandard (list(String.t)): DO NOT POPULATE THIS FIELD. It is never populated in responses from the platform, and is ignored in queries. It will be removed in a future version entirely. Defaults to `nil`.
  - dataStreamId (String.t): A unique identifier for the data stream produced by this data source. The identifier includes:
  ...
  Finally, the developer project number is obfuscated when read by any REST or Android client that did not create the data source. Only the data source creator will see the developer project number in clear and normal form. Defaults to `nil`.
  - dataStreamName (String.t): The stream name uniquely identifies this particular data source among other data sources of the same type from the same underlying producer. Setting the stream name is optional, but should be done whenever an application exposes two streams for the same data type, or when a device has two equivalent sensors. Defaults to `nil`.
  - dataType (GoogleApi.Fitness.V1.Model.DataType.t): The data type defines the schema for a stream of data being collected by, inserted into, or queried from the Fitness API. Defaults to `nil`.
  - device (GoogleApi.Fitness.V1.Model.Device.t): Representation of an integrated device (such as a phone or a wearable) that can hold sensors. Defaults to `nil`.
  - name (String.t): An end-user visible name for this data source. Defaults to `nil`.
  - type (String.t): A constant describing the type of this data source. Indicates whether this data source produces raw or derived data. Defaults to `nil`.
  """
