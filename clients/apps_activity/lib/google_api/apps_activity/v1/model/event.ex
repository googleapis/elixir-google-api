...
defmodule GoogleApi.AppsActivity.V1.Model.Event do
  ...
  - additionalEventTypes ([String.t]): Additional event types. Some events may have multiple types when multiple actions are part of a single event. For example, creating a document, renaming it, and sharing it may be part of a single file-creation event. Defaults to: `null`.
    - Enum - one of 
  - eventTimeMillis (String.t): The time at which the event occurred formatted as Unix time in milliseconds. Defaults to: `null`.
  ...
  - permissionChanges ([PermissionChange]): Extra information for permissionChange type events, such as the user or group the new permission applies to. Defaults to: `null`.
  - primaryEventType (String.t): The main type of event that occurred. Defaults to: `null`.
