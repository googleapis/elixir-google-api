...
defmodule GoogleApi.DFAReporting.V33.Api.ChangeLogs do
  ...
  *   `{:error, info}` on failure
  ...
  @spec dfareporting_change_logs_get(
          ...
          keyword(),
          keyword()
        ) :: {:ok, GoogleApi.DFAReporting.V33.Model.ChangeLog.t()} | {:error, Tesla.Env.t()}
  ...
  *   `optional_params` (*type:* `keyword()`) - Optional parameters
      ...
      *   `:minChangeTime` (*type:* `String.t`) - Select only change logs whose change time is before the specified minChangeTime.The time should be formatted as an RFC3339 date/time string. For example, for 10:54 PM on July 18th, 2015, in the America/New York time zone, the format is "2015-07-18T22:54:00-04:00". In other words, the year, month, day, the letter T, the hour (24-hour clock system), minute, second, and then the time zone offset.
