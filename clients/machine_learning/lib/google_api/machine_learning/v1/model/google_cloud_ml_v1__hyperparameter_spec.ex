...
defmodule GoogleApi.MachineLearning.V1.Model.GoogleCloudMlV1HyperparameterSpec do
  ...
  - maxFailedTrials (integer()): Optional. How many failed trials that need to be seen before failing the hyperparameter tuning job. User can specify this field to override the default failing criteria for CloudML Engine hyperparameter tuning jobs.  Defaults to zero, which means to let the service decide when a hyperparameter job should fail. Defaults to: `null`.
