...
defmodule GoogleApi.MachineLearning.V1.Model.GoogleCloudMlV1Model do
  ...
  - onlinePredictionConsoleLogging (boolean()): Optional. If true, enables logging of stderr and stdout streams for online prediction in Stackdriver Logging. These can be more verbose than the standard access logs (see &#x60;online_prediction_logging&#x60;) and thus can incur higher cost. However, they are helpful for debugging. Note that since Stackdriver logs may incur a cost, particularly if the total QPS in your project is high, be sure to estimate your costs before enabling this flag.  Default is false. Defaults to: `null`.
  - onlinePredictionLogging (boolean()): Optional. If true, online prediction access logs are sent to StackDriver Logging. These logs are like standard server access logs, containing information like timestamp and latency for each request. Note that Stackdriver logs may incur a cost, particular if the total QPS in your project is high.  Default is false. Defaults to: `null`.
