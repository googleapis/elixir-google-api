...
defmodule GoogleApi.LifeSciences.V2beta.Model.Action do
  ...
  *   `alwaysRun` (*type:* `boolean()`, *default:* `nil`) - By default, after an action fails, no further actions are run. This flag
      ...
      fails.
  *   `commands` (*type:* `list(String.t)`, *default:* `nil`) - If specified, overrides the `CMD` specified in the container. If the
      container also has an `ENTRYPOINT` the values are used as entrypoint
      arguments. Otherwise, they are used as a command and arguments to run
      inside the container.
  *   `containerName` (*type:* `String.t`, *default:* `nil`) - An optional name for the container. The container hostname will be set to
      this name, making it useful for inter-container communication. The name
      must contain only upper and lowercase alphanumeric characters and hyphens
      and cannot start with a hyphen.
  *   `credentials` (*type:* `GoogleApi.LifeSciences.V2beta.Model.Secret.t`, *default:* `nil`) - If the specified image is hosted on a private registry other than Google
  ...
  *   `disableStandardErrorCapture` (*type:* `boolean()`, *default:* `nil`) - A small portion of the container's standard error stream is typically
      captured and returned inside the `ContainerStoppedEvent`. Setting this
      flag disables this functionality.
  *   `enableFuse` (*type:* `boolean()`, *default:* `nil`) - Enable access to the FUSE device for this action. Filesystems can then
  ...
  *   `environment` (*type:* `map()`, *default:* `nil`) - The environment to pass into the container. This environment is merged
  ...
  *   `ignoreExitStatus` (*type:* `boolean()`, *default:* `nil`) - Normally, a non-zero exit status causes the pipeline to fail. This flag
      allows execution of other actions to continue instead.
  *   `imageUri` (*type:* `String.t`, *default:* `nil`) - Required. The URI to pull the container image from. Note that all images referenced
  ...
  *   `labels` (*type:* `map()`, *default:* `nil`) - Labels to associate with the action. This field is provided to assist
      workflow engine authors in identifying actions (for example, to indicate
      what sort of action they perform, such as localization or debugging).
      They are returned in the operation metadata, but are otherwise ignored.
  *   `mounts` (*type:* `list(GoogleApi.LifeSciences.V2beta.Model.Mount.t)`, *default:* `nil`) - A list of mounts to make available to the action.
  ...
  *   `pidNamespace` (*type:* `String.t`, *default:* `nil`) - An optional identifier for a PID namespace to run the action inside.
      Multiple actions should use the same string to share a namespace.  If
      unspecified, a separate isolated namespace is used.
  *   `portMappings` (*type:* `map()`, *default:* `nil`) - A map of containers to host port mappings for this container. If the
  ...
  *   `publishExposedPorts` (*type:* `boolean()`, *default:* `nil`) - Exposes all ports specified by `EXPOSE` statements in the container. To
      discover the host side port numbers, consult the `ACTION_STARTED` event
      in the operation metadata.
  *   `runInBackground` (*type:* `boolean()`, *default:* `nil`) - This flag allows an action to continue running in the background while
  ...
  *   `timeout` (*type:* `String.t`, *default:* `nil`) - The maximum amount of time to give the action to complete. If the action
