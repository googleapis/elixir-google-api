...
defmodule GoogleApi.CloudIot.V1.Model.Device do
  ...
  example, the sensor is generating bad data and needs maintenance. Defaults to `nil`.
  ...
  `DeviceManager.ModifyCloudToDeviceConfig` method. Defaults to `nil`.
  ...
  - lastHeartbeatTime (DateTime.t): [Output only] The last time an MQTT `PINGREQ` was received. This field
  applies only to devices connecting through MQTT. MQTT clients usually only
  send `PINGREQ` messages if the connection is idle, and no other messages
  have been sent. Timestamps are periodically collected and written to
  storage; they may be stale by a few minutes. Defaults to `nil`.
  ...
  - metadata (map()): The metadata key-value pairs assigned to the device. This metadata is not
