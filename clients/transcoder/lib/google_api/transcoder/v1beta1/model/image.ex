...
defmodule GoogleApi.Transcoder.V1beta1.Model.Image do
  ...
  *   `resolution` (*type:* `GoogleApi.Transcoder.V1beta1.Model.NormalizedCoordinate.t`, *default:* `nil`) - Normalized image resolution, based on output video resolution. Valid values: `0.0`–`1.0`. To respect the original image aspect ratio, set either `x` or `y` to `0.0`. To use the original image resolution, set both `x` and `y` to `0.0`.
