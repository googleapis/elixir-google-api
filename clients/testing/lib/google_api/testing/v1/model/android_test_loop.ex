...
defmodule GoogleApi.Testing.V1.Model.AndroidTestLoop do
  ...
  The default is determined by examining the application's manifest. Defaults to `nil`.
  - scenarioLabels (list(String.t)): The list of scenario labels that should be run during the test.
  The scenario labels should map to labels defined in the application's
  manifest. For example, player_experience and
  com.google.test.loops.player_experience add all of the loops labeled in the
  manifest with the com.google.test.loops.player_experience name to the
  execution.
  Scenarios can also be specified in the scenarios field. Defaults to `nil`.
  - scenarios (list(integer())): The list of scenarios that should be run during the test.
  The default is all test loops, derived from the application's
  manifest. Defaults to `nil`.
  """
