...
defmodule GoogleApi.AnalyticsReporting.V4.Model.ReportRequest do
  ...
  contain the same `cohortGroup` definition. Defaults to `nil`.
  - dateRanges (list(GoogleApi.AnalyticsReporting.V4.Model.DateRange.t)): Date ranges in the request. The request can have a maximum of 2 date
  ...
  contain the same `dateRanges` definition. Defaults to `nil`.
  - dimensionFilterClauses (list(GoogleApi.AnalyticsReporting.V4.Model.DimensionFilterClause.t)): The dimension filter clauses for filtering Dimension Values. They are
  logically combined with the `AND` operator. Note that filtering occurs
  before any dimensions are aggregated, so that the returned metrics
  represent the total for only the relevant dimensions. Defaults to `nil`.
  - dimensions (list(GoogleApi.AnalyticsReporting.V4.Model.Dimension.t)): The dimensions requested.
  Requests can have a total of 7 dimensions. Defaults to `nil`.
  - filtersExpression (String.t): Dimension or metric filters that restrict the data returned for your
  request. To use the `filtersExpression`, supply a dimension or metric on
  which to filter, followed by the filter expression. For example, the
  following expression selects `ga:browser` dimension which starts with
  Firefox; `ga:browser=~^Firefox`. For more information on dimensions
  and metric filters, see
  [Filters reference](https://developers.google.com/analytics/devguides/reporting/core/v3/reference#filters). Defaults to `nil`.
  - hideTotals (boolean()): If set to true, hides the total of all metrics for all the matching rows,
  for every date range. The default false and will return the totals. Defaults to `nil`.
  - hideValueRanges (boolean()): If set to true, hides the minimum and maximum across all matching rows.
  The default is false and the value ranges are returned. Defaults to `nil`.
  - includeEmptyRows (boolean()): If set to false, the response does not include rows if all the retrieved
  metrics are equal to zero. The default is false which will exclude these
  rows. Defaults to `nil`.
  - metricFilterClauses (list(GoogleApi.AnalyticsReporting.V4.Model.MetricFilterClause.t)): The metric filter clauses. They are logically combined with the `AND`
  operator.  Metric filters look at only the first date range and not the
  comparing date range. Note that filtering on metrics occurs after the
  metrics are aggregated. Defaults to `nil`.
  - metrics (list(GoogleApi.AnalyticsReporting.V4.Model.Metric.t)): The metrics requested.
  Requests must specify at least one metric. Requests can have a
  total of 10 metrics. Defaults to `nil`.
  - orderBys (list(GoogleApi.AnalyticsReporting.V4.Model.OrderBy.t)): Sort order on output rows. To compare two rows, the elements of the
  following are applied in order until a difference is found.  All date
  ranges in the output get the same row order. Defaults to `nil`.
  ...
  even if you set `pageSize` to a higher value. Defaults to `nil`.
  - pageToken (String.t): A continuation token to get the next page of the results. Adding this to
  the request will return the rows after the pageToken. The pageToken should
  be the value returned in the nextPageToken parameter in the response to
  the GetReports request. Defaults to `nil`.
  - pivots (list(GoogleApi.AnalyticsReporting.V4.Model.Pivot.t)): The pivot definitions. Requests can have a maximum of 2 pivots. Defaults to `nil`.
  - samplingLevel (String.t): The desired report
  ...
  - segments (list(GoogleApi.AnalyticsReporting.V4.Model.Segment.t)): Segment the data returned for the request. A segment definition helps look
  at a subset of the segment request. A request can contain up to four
  segments. Every [ReportRequest](#ReportRequest) within a
  `batchGet` method must contain the same `segments` definition. Requests
  with segments must have the `ga:segment` dimension. Defaults to `nil`.
  - viewId (String.t): The Analytics
  [view ID](https://support.google.com/analytics/answer/1009618)
  from which to retrieve data. Every [ReportRequest](#ReportRequest)
  within a `batchGet` method must contain the same `viewId`. Defaults to `nil`.
  """
