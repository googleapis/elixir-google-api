...
defmodule GoogleApi.Spanner.V1.Model.PartialResultSet do
  ...
  to obtain a complete field value. Defaults to `nil`.
  - metadata (GoogleApi.Spanner.V1.Model.ResultSetMetadata.t): Metadata about the result set, such as row type information.
  Only present in the first response. Defaults to `nil`.
  - resumeToken (String.t): Streaming calls might be interrupted for a variety of reasons, such
  as TCP connection loss. If this occurs, the stream of results can
  be resumed by re-sending the original request and including
  `resume_token`. Note that executing any other transaction in the
  same session invalidates the token. Defaults to `nil`.
  - stats (GoogleApi.Spanner.V1.Model.ResultSetStats.t): Query plan and execution statistics for the statement that produced this
  streaming result set. These can be requested by setting
  ExecuteSqlRequest.query_mode and are sent
  only once with the last response in the stream.
  This field will also be present in the last response for DML
  statements. Defaults to `nil`.
  - values (list(any())): A streamed result set consists of a stream of values, which might
  ...
  * `object`: concatenate the (field name, field value) pairs. If a
  ...
  field value `"World" = "W" + "orl" + "d"`. Defaults to `nil`.
  """
