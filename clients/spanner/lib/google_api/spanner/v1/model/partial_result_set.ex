...
defmodule GoogleApi.Spanner.V1.Model.PartialResultSet do
  ...
  *   `chunkedValue` (*type:* `boolean()`, *default:* `nil`) - If true, then the final value in values is chunked, and must
  ...
  *   `resumeToken` (*type:* `String.t`, *default:* `nil`) - Streaming calls might be interrupted for a variety of reasons, such
      as TCP connection loss. If this occurs, the stream of results can
      be resumed by re-sending the original request and including
      `resume_token`. Note that executing any other transaction in the
      same session invalidates the token.
  *   `stats` (*type:* `GoogleApi.Spanner.V1.Model.ResultSetStats.t`, *default:* `nil`) - Query plan and execution statistics for the statement that produced this
      streaming result set. These can be requested by setting
      ExecuteSqlRequest.query_mode and are sent
      only once with the last response in the stream.
      This field will also be present in the last response for DML
      statements.
  *   `values` (*type:* `list(any())`, *default:* `nil`) - A streamed result set consists of a stream of values, which might
