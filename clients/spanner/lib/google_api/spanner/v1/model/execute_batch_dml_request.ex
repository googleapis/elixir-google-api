...
defmodule GoogleApi.Spanner.V1.Model.ExecuteBatchDmlRequest do
  ...
  - seqno (String.t): A per-transaction sequence number used to identify this request. This is
  used in the same space as the seqno in
  ExecuteSqlRequest. See more details
  in ExecuteSqlRequest. Defaults to `nil`.
  - statements (list(GoogleApi.Spanner.V1.Model.Statement.t)): The list of statements to execute in this batch. Statements are executed
  serially, such that the effects of statement i are visible to statement
  i+1. Each statement must be a DML statement. Execution will stop at the
  first failed statement; the remaining statements will not run.

  REQUIRES: `statements_size()` > 0. Defaults to `nil`.
  - transaction (GoogleApi.Spanner.V1.Model.TransactionSelector.t): The transaction to use. A ReadWrite transaction is required. Single-use
  transactions are not supported (to avoid replay).  The caller must either
  supply an existing transaction ID or begin a new transaction. Defaults to `nil`.
  """
