...
defmodule GoogleApi.Games.V1.Api.Players do
  ...
  *   `{:ok, %GoogleApi.Games.V1.Model.ScopedPlayerIds{}}` on success
  *   `{:error, info}` on failure
  """
  @spec games_players_get_scoped_player_ids(Tesla.Env.client(), keyword(), keyword()) ::
          {:ok, GoogleApi.Games.V1.Model.ScopedPlayerIds.t()}
          | {:ok, Tesla.Env.t()}
          | {:ok, list()}
          | {:error, any()}
  def games_players_get_scoped_player_ids(connection, optional_params \\ [], opts \\ []) do
    optional_params_config = %{
      :"$.xgafv" => :query,
      :access_token => :query,
      :alt => :query,
    ...
    request =
      Request.new()
      |> Request.method(:get)
      |> Request.url("/games/v1/players/me/scopedIds", %{})
      |> Request.add_optional_params(optional_params_config, optional_params)
      |> Request.library_version(@library_version)

    connection
    |> Connection.execute(request)
    |> Response.decode(opts ++ [struct: %GoogleApi.Games.V1.Model.ScopedPlayerIds{}])
  end
