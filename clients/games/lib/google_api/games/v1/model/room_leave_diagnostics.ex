# Copyright 2019 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the &quot;License&quot;);
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an &quot;AS IS&quot; BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

# NOTE: This class is auto generated by the elixir code generator program.
# Do not edit the class manually.

defmodule GoogleApi.Games.V1.Model.RoomLeaveDiagnostics do
  @moduledoc """
  This is a JSON template for room leave diagnostics.

  ## Attributes

  - androidNetworkSubtype (integer()): Android network subtype. http://developer.android.com/reference/android/net/NetworkInfo.html#getSubtype() Defaults to `nil`.
  - androidNetworkType (integer()): Android network type. http://developer.android.com/reference/android/net/NetworkInfo.html#getType() Defaults to `nil`.
  - iosNetworkType (integer()): iOS network type as defined in Reachability.h. Defaults to `nil`.
  - kind (String.t): Uniquely identifies the type of this resource. Value is always the fixed string games#roomLeaveDiagnostics. Defaults to `games#roomLeaveDiagnostics`.
  - networkOperatorCode (String.t): The MCC+MNC code for the client's network connection. On Android: http://developer.android.com/reference/android/telephony/TelephonyManager.html#getNetworkOperator() On iOS, see: https://developer.apple.com/library/ios/documentation/NetworkingInternet/Reference/CTCarrier/Reference/Reference.html Defaults to `nil`.
  - networkOperatorName (String.t): The name of the carrier of the client's network connection. On Android: http://developer.android.com/reference/android/telephony/TelephonyManager.html#getNetworkOperatorName() On iOS: https://developer.apple.com/library/ios/documentation/NetworkingInternet/Reference/CTCarrier/Reference/Reference.html#//apple_ref/occ/instp/CTCarrier/carrierName Defaults to `nil`.
  - peerSession (list(GoogleApi.Games.V1.Model.PeerSessionDiagnostics.t)): Diagnostics about all peer sessions. Defaults to `nil`.
  - socketsUsed (boolean()): Whether or not sockets were used. Defaults to `nil`.
  """

  use GoogleApi.Gax.ModelBase

  @type t :: %__MODULE__{
          :androidNetworkSubtype => integer(),
          :androidNetworkType => integer(),
          :iosNetworkType => integer(),
          :kind => String.t(),
          :networkOperatorCode => String.t(),
          :networkOperatorName => String.t(),
          :peerSession => list(GoogleApi.Games.V1.Model.PeerSessionDiagnostics.t()),
          :socketsUsed => boolean()
        }

  field(:androidNetworkSubtype)
  field(:androidNetworkType)
  field(:iosNetworkType)
  field(:kind)
  field(:networkOperatorCode)
  field(:networkOperatorName)
  field(:peerSession, as: GoogleApi.Games.V1.Model.PeerSessionDiagnostics, type: :list)
  field(:socketsUsed)
end

defimpl Poison.Decoder, for: GoogleApi.Games.V1.Model.RoomLeaveDiagnostics do
  def decode(value, options) do
    GoogleApi.Games.V1.Model.RoomLeaveDiagnostics.decode(value, options)
  end
end

defimpl Poison.Encoder, for: GoogleApi.Games.V1.Model.RoomLeaveDiagnostics do
  def encode(value, options) do
    GoogleApi.Gax.ModelBase.encode(value, options)
  end
end
