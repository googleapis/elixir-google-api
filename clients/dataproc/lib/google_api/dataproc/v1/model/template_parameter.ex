...
defmodule GoogleApi.Dataproc.V1.Model.TemplateParameter do
  ...
  - description (String.t): Optional. Brief description of the parameter. Must not exceed 1024 characters. Defaults to `nil`.
  - fields (list(String.t)): Required. Paths to all fields that the parameter replaces. A field is allowed to appear in at most one parameter's list of field paths.A field path is similar in syntax to a google.protobuf.FieldMask. For example, a field path that references the zone field of a workflow template's cluster selector would be specified as placement.clusterSelector.zone.Also, field paths can reference fields using the following syntax:
  ...
  jobs'step-id'.sparkJob.args Defaults to `nil`.
  - name (String.t): Required. Parameter name. The parameter name is used as the key, and paired with the parameter value, which are passed to the template when the template is instantiated. The name must contain only capital letters (A-Z), numbers (0-9), and underscores (_), and must not start with a number. The maximum length is 40 characters. Defaults to `nil`.
  - validation (GoogleApi.Dataproc.V1.Model.ParameterValidation.t): Optional. Validation rules to be applied to this parameter's value. Defaults to `nil`.
  """
