...
defmodule GoogleApi.Dataproc.V1.Model.ClusterConfig do
  ...
  - configBucket (String.t): Optional. A Google Cloud Storage bucket used to stage job dependencies, config files, and job driver console output. If you do not specify a staging bucket, Cloud Dataproc will determine a Cloud Storage location (US, ASIA, or EU) for your cluster's staging bucket according to the Google Compute Engine zone where your cluster is deployed, and then create and manage this project-level, per-location bucket (see Cloud Dataproc staging bucket). Defaults to `nil`.
  - encryptionConfig (GoogleApi.Dataproc.V1.Model.EncryptionConfig.t): Optional. Encryption settings for the cluster. Defaults to `nil`.
  - gceClusterConfig (GoogleApi.Dataproc.V1.Model.GceClusterConfig.t): Optional. The shared Compute Engine config settings for all instances in a cluster. Defaults to `nil`.
  - initializationActions (list(GoogleApi.Dataproc.V1.Model.NodeInitializationAction.t)): Optional. Commands to execute on each node after config is completed. By default, executables are run on master and all worker nodes. You can test a node's role metadata to run an executable on a master or worker node, as shown below using curl (you can also use wget):
  ...
  Defaults to `nil`.
  - masterConfig (GoogleApi.Dataproc.V1.Model.InstanceGroupConfig.t): Optional. The Compute Engine config settings for the master instance in a cluster. Defaults to `nil`.
  - secondaryWorkerConfig (GoogleApi.Dataproc.V1.Model.InstanceGroupConfig.t): Optional. The Compute Engine config settings for additional worker instances in a cluster. Defaults to `nil`.
  - securityConfig (GoogleApi.Dataproc.V1.Model.SecurityConfig.t): Optional. Security settings for the cluster. Defaults to `nil`.
  - softwareConfig (GoogleApi.Dataproc.V1.Model.SoftwareConfig.t): Optional. The config settings for software inside the cluster. Defaults to `nil`.
  - workerConfig (GoogleApi.Dataproc.V1.Model.InstanceGroupConfig.t): Optional. The Compute Engine config settings for worker instances in a cluster. Defaults to `nil`.
  """
